...
class Preferences:
    def __init__(self, params: dict, path_to_prefs: str):
        self.params = params
        ...
        self.formatter = Formatter(principles_csv=params["principles"], few_shots=params["few_shots"], selected=params["statement_ids"])
    ...
    def make_df(self):
        ...
        merged_df = pref_df.merge(self.params["principles"], left_on='principle_id', right_on='id', suffixes=('', '_y'))
    ...
    def generate(self):
        ...
        if isinstance(params["data"], (Dataset, DatasetDict)):
            data = params["data"]
        elif is_json_or_jsonl(params["data"]):
            data = load_dataset('json', data_files=params["data"])['train'] 
        else:
